...
if [ "$INTRO" = 1 ]; then
  {
    ...
    echo "════════════════════════════════════════════════════════════════════"
    echo " What         : OracleGuard is a thin governed-disbursement adapter"
    echo "                on the KZS stack (Katiba · Ziranity · Shadowbox)."
    echo "                Given a pre-approved allocation, a fixed policy, and"
    echo "                a live oracle fact, it builds a canonical intent,"
    echo "                submits it to Ziranity BFT consensus which runs"
    echo "                OracleGuard's three-gate closure (anchor → registry"
    echo "                → grant) inside every validator, and — on Authorized"
    echo "                — emits a typed effect Cardano settlement executes"
    echo "                verbatim."
    echo
    echo "                Core properties: deterministic (same inputs → same"
    echo "                bytes), BFT-agreed (4 validators under a Quorum"
    echo "                Certificate — QC), over-cap-impossible (cap enforced"
    echo "                at consensus), independently replayable offline from"
    echo "                recorded evidence."
    echo
    echo " Flow         : 1. show policy + derive policy_ref (sha256 canonical)"
    echo "                2. live Charli3 pull (on-demand oracle aggregation)"
    echo "                3. build canonical DisbursementIntentV1 from live price"
    echo "                4. Ziranity 4-node BFT commits AuthorizationResult bytes"
    echo "                5. Cardano Preprod settlement on Authorized (real tx)"
    echo "                6. offline verifier replays live bundles → CLEAN"
    echo "                7. policy rotation: changed rule → new policy_ref"
    echo
    echo " Out of scope : on-chain Katiba anchoring  ·  Shadowbox registered"
    echo "                bundles  ·  multi-asset (ADA only)  ·  multi-authority"
    echo "                routing (single disbursement Object Consensus Unit — OCU)"
    echo
    ...
    echo
    echo " Security     : Preprod only. Mnemonics via off-screen \`read -s\`, never"
    echo "                on disk. Proof-of-concept — not production-audited."
    echo "════════════════════════════════════════════════════════════════════"
  } | tee -a "$TRANSCRIPT_FILE"
fi
